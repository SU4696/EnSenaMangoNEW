...
//  EnSena
//
//  Created by Daniel Jauregui Gomez on 28/11/22.
//
...
struct LocalNotificationManager {
    ...
    static private func requestPermission () -> Void {
        UNUserNotificationCenter
            .current()
            .requestAuthorization(options : [.alert,.badge,.alert]){granted, error
                in
    ...
    static private func scheduleNotifications(duration: Int, of type: LocalNotificationDurationType, repeats: Bool, userInfo: [AnyHashable: Any]){
        var seconds = 0
        switch type{
        case .seconds:
            seconds = duration
    ...
    static func setNotification (duration : Int, of type: LocalNotificationDurationType, repeats: Bool, title: String, body: String, userinfo: [AnyHashable:Any]){
        requestPermission()
        addNotification(title:title, body:body)
        scheduleNotifications(duration: duration, of: type, repeats: repeats, userInfo: userinfo)
    }
